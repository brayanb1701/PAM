...
def get_data(p):
    ...
    with open(p.output_file, 'w') as f:
        for i in range(nbnds):
            for j in range(nqpts):
                f.write(f"{D1[j]:.4f}\t{F1[j,i]:.4f}\t{Jxyz[0,j,i]:.4f}\t{Jxyz[1,j,i]:.4f}\t{Jxyz[2,j,i]:.4f}\n")
            f.write(f"\n")
...
def pam_plot(p):
    ...
    for ii in range(nsubs):
        ...
        ax.axhline(y=0, ls='-', lw=0.5, color='k', alpha=0.6)
        if p.normalization == 'per_direction':
            norm = mpl.colors.Normalize(vmin=Jxyz[which_j].min(), vmax=Jxyz[which_j].max())
        else: 
            norm = mpl.colors.Normalize(vmin=Jxyz.min(), vmax=Jxyz.max())
            print(Jxyz.min(),Jxyz[which_j].min())
            print(Jxyz.max(),Jxyz[which_j].max())
        s_m = mpl.cm.ScalarMappable(cmap=pam_cmaps[which_j], norm=norm)
        s_m.set_array([Jxyz[which_j]])
...
def parse_cml_args(cml):
    ...
    arg.add_argument('-a', '--action', dest='action', action='store', type=str,
                     default='bands', choices=['data', 'bands', 'band'],
    ...
    arg.add_argument('-od', '--output_data', dest='output_file', type=str, default='', help='Output data file')

    arg.add_argument('-n', '--normalization', dest='normalization', type=str, default='per_direction', choices=['per_band', 'all'], help='Decide how to normalize')

    arg.add_argument('-idx', '--index', dest='idx', type=int, default=0, help='Band number to plot')
